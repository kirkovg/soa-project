...
public class BookpictureResource {
    ...
    public ResponseEntity<BookpictureDTO> createBookpicture(@RequestBody BookpictureDTO bookpictureDTO) throws URISyntaxException {
        ...
        if (bookpictureDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new bookpicture cannot already have an ID")).body(null);
        }
    ...
    @Timed
    public ResponseEntity<List<BookpictureDTO>> getAllBookpictures(@ApiParam Pageable pageable) {
